...
exports.insertBqData = insertBqData;
function insertBqData(data, table, id, dataset){
	...
	bigquery.job.load(prjId, dsName, tableName, queue, function(e,r,d){
		...
		if(data.length > 0) {
			...
			insertBqData(data, tableName, prjId, dsName);
		}
	});
}

exports.createBqTable = createBqTable;
function createBqTable(info, name, dataset, project, fn){
	...
	info.forEach(function(value, index){
		...
		tmp_schema.type = bqtype.type[value.type] || 'STRING';
...
exports.getBqTable = getBqTable;
function getBqTable(name, dataset, project, fn){
	if(dataset && typeof(dataset) === 'function') var fn = dataset;
	if(project && typeof(project) === 'function') var fn = project;
